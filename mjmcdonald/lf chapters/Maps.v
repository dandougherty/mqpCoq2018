...
Proof.
  intros x y. apply iff_reflect. symmetry. apply beq_string_true_iff.
Qed.
(** [] *)
...
Theorem t_update_same : forall X x (m : total_map X),
    m & { x --> m x } = m.
Proof.
  intros X x m. unfold t_update. apply functional_extensionality. intros y.
...
Theorem t_update_permute : forall (X:Type) v1 v2 x1 x2
  ...
  =  m & { x1 --> v1 ; x2 --> v2 }.
Proof.
  intros X v1 v2 x1 x2 m H. apply functional_extensionality. intros x.
  unfold t_update. destruct (beq_stringP x1 x); destruct (beq_stringP x2 x).
