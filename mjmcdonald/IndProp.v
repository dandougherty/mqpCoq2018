...
Proof.
  ...
  - intros m H1. simpl. apply H1.
  - intros m H1. simpl. apply ev_SS. apply IHev. apply H1.
Qed.
(** [] *)
...
Proof.
  ...
  - simpl in H. apply H.
  - inversion H. apply IHev. apply H2.
Qed.
(** [] *)
...
Proof.
  ...
  - assert (H2 : n + m + (n + p) = (n + n) + (m + p)).
    ...
    rewrite H2 in H. apply ev_ev__ev with (n + n) (m + p) in H.
    + apply H.
...
Proof.
  intros m n o H H1. induction H1.
  - apply H.
...
Proof.
  ...
  - apply le_trans with (m:=(S n1)) (n:=(S(n1 + n2))) (o:=m).
    + rewrite S_distr. apply le_plus_l.
    + apply H.
  - apply le_trans with (m:=(S n2)) (n:=(S(n1 + n2))) (o:=m).
    + rewrite plus_comm. rewrite S_distr. apply le_plus_l.
    + apply H.
Qed.
...
Theorem leb_complete : forall n m,
  leb n m = true -> n <= m.
Proof.
  intros n m H. generalize dependent m. induction n; induction m.
  - reflexivity.
  - intros H. apply le_S. apply IHm. apply H.
  - intros H. inversion H.
  - intros H. apply n_le_m__Sn_le_Sm. apply IHn. apply H.
Qed.
...
Proof.
  intros n m o H H1. apply leb_complete in H. 
...
Only R 1 1 2
It doesn't use c4 or c5 so no to both
*)
(** [] *)
...
Proof.
  ...
  - intros H. generalize dependent m. generalize dependent n. 
    induction o; destruct m; destruct n; intros H.
    + apply c1.
    + inversion H.
    + inversion H.
    + inversion H.
...
Inductive subseq : list nat -> list nat -> Prop :=
  ...
  | subseq_match : forall x l1 l2, subseq l1 l2 -> subseq (x::l1) (x::l2)
...
Proof.
  ...
  induction H.
  - apply subseq_nil.
  - apply subseq_match. apply IHsubseq.
  - apply subseq_fail. apply IHsubseq.
Qed.
...
Proof.
  intros l1 l2 l3 H H1. generalize dependent H. generalize dependent l1. induction H1.
  - intros. inversion H. apply subseq_nil.
  - intros. inversion H.
    + apply subseq_nil.
    + apply subseq_match. apply IHsubseq. apply H3.
    + apply subseq_fail. apply IHsubseq. apply H3.
  - intros. apply subseq_fail. apply IHsubseq. apply H.
Qed.
...
Proof.
  ...
  - generalize dependent s1. induction s2; simpl; intros s1 H; inversion H.
    + reflexivity.
    + inversion H3. simpl. apply f_equal. apply IHs2. apply H4.
  - generalize dependent s2. induction s1; simpl; intros s2 H.
    + destruct H. apply MEmpty.
    + replace (x :: s1) with ([x] ++ s1). rewrite <- H. apply MApp.
      * apply MChar.
...
Proof.
  ...
  - intros. induction re.
    ...
    + simpl in H. apply orb_true_iff in H. destruct H.
      * apply IHre1 in H as []. exists x. apply MUnionL. apply H.
      * apply IHre2 in H as []. exists x. apply MUnionR. apply H.
    + exists []. apply MStar0.
Qed.
(** [] *)
...
Lemma MStar'' : forall T (s : list T) (re : reg_exp),
  ...
  exists ss : list (list T),
    ...
    /\ forall s', In s' ss -> s' =~ re.
Proof.
  intros. remember (Star re) as re'. induction H; inversion Heqre'. 
  - exists []. split.
    + reflexivity.
    + intros s'. contradiction.
  - induction s1.
    + simpl. apply IHexp_match2. apply Heqre'.
    + apply IHexp_match2 in Heqre'. destruct Heqre'. destruct H1. exists ((x :: s1) :: x0). split.
...
Proof.
  ...
  induction Hmatch
    as [ | x | s1 re1 s2 re2 Hmatch1 IH1 Hmatch2 IH2
       ...
       | re | s1 s2 re Hmatch1 IH1 Hmatch2 IH2 ].
  - (* MEmpty *)
    simpl. intros H. inversion H. (* omega. *)
  - (* MChar *)
    simpl. omega. (* intros H. inversion H. inversion H0. *)
  - (* MApp *)
    simpl. intros H. 
...
Proof.
  intros P b H. split.
  - intros H1. destruct H. reflexivity. unfold not in H. apply H in H1. inversion H1.
  - intros H1. destruct H. apply H. inversion H1.
Qed.
(** [] *)
...
Proof.
  ...
  - simpl in H. destruct (beq_natP n x).
    + inversion H.
    + apply IHl in H. intros H1. inversion H1.
...
Inductive nostutter {X:Type} : list X -> Prop :=
  ...
  | nostutter_two : forall x1 x2 l, x1 <> x2 -> nostutter (x2 :: l) -> nostutter (x1 :: x2 :: l).
...
Inductive merge {X:Type} : list X -> list X -> list X -> Prop :=
  | merge_nil : merge [] [] []
  | merge_l1  : forall x1 l1 l2 l3, merge l1 l2 l3 -> merge (x1 :: l1) l2 (x1 :: l3)
  | merge_l2  : forall x2 l1 l2 l3, merge l1 l2 l3 -> merge l1 (x2 :: l2) (x2 :: l3).

Example merge1 : merge [1;6;2] [4;3] [1;4;6;2;3].
Proof.
  apply merge_l1. apply merge_l2. apply merge_l1. apply merge_l1. apply merge_l2. apply merge_nil.
Qed.

Theorem merge_filter : forall X (test: X->bool) (l l1 l2: list X),
  merge l1 l2 l ->
...
Theorem filter_challenge_2 : forall (test: nat->bool) (l l1 : list nat),
  subseq l1 l -> 
  All (fun x => test x = true) l1 -> 
  length l1 <= length (filter test l).
Proof.
  intros test l l1 H H1.
Admitted.
...
Inductive palindrome {X : Type} : list X -> Prop :=
  ...
  | pal_one : forall x, palindrome [x]
  | pal_app : forall x l, palindrome l -> palindrome (x :: l ++ [x]).

Example pal_test : palindrome [1;2;3;2;1].
Proof.
  replace [1;2;3;2;1] with (1 :: [2;3;2] ++ [1]).
  apply pal_app.
  replace [2;3;2] with (2 :: [3] ++ [2]).
  apply pal_app.
  apply pal_one.
...
Theorem pal_conv : forall X (l : list X),
...
Inductive disjoint {X : Type} : list X -> list X -> Prop :=
  | disjoint_nil : forall l2, disjoint [] l2
  | disjoint_x   : forall x l1 l2, ~(In x l2) -> disjoint l1 l2 -> disjoint (x :: l1) l2.
...
Inductive nodup {X : Type} : list X -> Prop :=
  | nodup_nil : nodup []
  | nodup_x   : forall x l1, ~(In x l1) -> nodup l1 -> nodup (x :: l1).
