...
Proof.
  ...
  - destruct c.
    + reflexivity.
    + intros h. rewrite -> false_and.
      * rewrite -> h. reflexivity.
      * reflexivity.
  - destruct c.
    + intros h. rewrite -> true_or.
      * rewrite <- h. reflexivity.
...
Example test_incr0: (incr Z) = P Z.
Proof. reflexivity. Qed.

Example test_incr1: (incr (P Z)) = D (P Z).
Proof. reflexivity. Qed.

Example test_incr2: (incr (D (P Z))) = P (D (P Z)).
Proof. reflexivity. Qed.

Example test_incr3: (incr (P (D (P Z)))) = D (D (P Z)).
Proof. reflexivity. Qed.

Example test_incr4: (incr (D (D (P Z)))) = (P (D (D (P Z)))).
Proof. reflexivity. Qed.
...
Example test_bin: (bin_to_nat Z) = O.
Proof. reflexivity. Qed.

Example test_bin1: (bin_to_nat (P Z)) = S O.
Proof. reflexivity. Qed.

Example test_bin2: (bin_to_nat (D (P Z))) = S (S O).
Proof. reflexivity. Qed.

Example test_bin3: (bin_to_nat (P (D (P Z)))) = S (S (S O)).
Proof. reflexivity. Qed.

Example test_bin4: (bin_to_nat (D (D (P Z)))) = S (S (S (S O))).
Proof. reflexivity. Qed.

Example test_both: S (bin_to_nat (D (P Z))) = (bin_to_nat (incr (D (P Z)))).
Proof. reflexivity. Qed.
