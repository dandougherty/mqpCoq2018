...
Fixpoint nth_error {X : Type} (l : list X) (n : nat)
  ...
  match l with
  | [] => None
  | h :: l' => if beq_nat n O then Some h else nth_error l' (pred n)
  end.
...
Definition evn_g7 (n:nat) : bool :=
    if evenb n then (negb (leb (minus n 7) 0))
               else false.
...
Fixpoint filter_neg {X:Type} (test: X->bool) (l:list X)
                : (list X) :=
  match l with
  | []     => []
  | h :: t => if test h then filter_neg test t 
                        else h :: filter_neg test t      
  end.
...
Definition partition {X : Type}
                     ...
                     (l : list X)
                   : list X * list X :=
 (filter test l, filter_neg test l).
...
(* Looked up HELP for this lemma and theorem, did not finish it*)



(* Lemma map_helper : forall (X Y : Type) (f: X -> Y) (l : list X) (x : X),
  map f (helper l x) = helper (map f l) (f x).
Proof.
  intros X Y f l x. 
  induction l as [| z l'].
    - reflexivity.
...
(* HAD SOME ISSUES WITH CURCH NUMERALS, LOOKE UP STUFF *)
